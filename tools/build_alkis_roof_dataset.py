...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Download and process DOP images and roof polygon data."
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument(
        "--wms-url",
        type=str,
        default="https://www.wms.nrw.de/geobasis/wms_nw_dop",
        help="WMS service URL for DOP data.",
    )
    parser.add_argument(
        "--layer-name",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of worker threads for fetching WMS tiles.",
    )
    parser.add_argument(
        "--layer-gsd",
        type=float,
        default=0.1,
        help="Ground sampling distance (m/pixel) for the WMS layer.",
    )
    parser.add_argument(
        "--target-gsd",
        type=float,
        default=0.2,
        help="Target ground sampling distance (m/pixel) for processing tiles.",
    )
    parser.add_argument(
        "--img-width",
        type=int,
...
def fetch_and_save_dop(tile, tile_index, dop_output_dir, wms_url, layer_name, img_width, img_height):
...
def process_tile(index, tile_and_shapes, dop_output_dir, wms_url, layer_name, img_width, img_height, debug, debug_data):
    """Fetches a DOP image for the tile and updates the dictionary."""
    tile = tile_and_shapes["tile"]
    dop_name = fetch_and_save_dop(tile, index, dop_output_dir, wms_url, layer_name, img_width, img_height)
...
def main():
    ...
    # Calculate tile dimensions in map units using TARGET_GSD and image size.
    ...
    with ThreadPoolExecutor(max_workers=args.wms_workers) as executor:
        futures = {
            executor.submit(
                process_tile,
                i,
